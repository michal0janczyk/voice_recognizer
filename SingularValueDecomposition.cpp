...
SingularValueDecomposition::SingularValueDecomposition(const Matrix& Arg)
{
	// Derived from LINPACK code.
	// Initialize.
	std::vector<std::vector<double>> A = Arg.getArrayCopy();
	...
	*/
	int nu = std::min(m, n);
	s.resize(std::min(m + 1, n), 0.0);
	U.resize(m, std::vector<double>(nu, 0.0));
	V.resize(n, std::vector<double>(n, 0.0));
	...
	while (p > 0)
	{
		...
		else
		{
			...
			for (ks = p-1; ks >= k; ks--)
			{
				if (ks == k)
					break;
				double t = (ks != p ? std::abs(e[ks]) : 0.0) + (ks != k+1 ? std::abs(e[ks-1]) : 0.0);
...
{
	return Matrix(U, m, std::min(m + 1, n));
}
...
std::vector<double> SingularValueDecomposition::getSingularValues() const
{
	return s;
}
...
{
	Matrix X{n, n};
	std::vector<std::vector<double>>& S = X.getArray();
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
			S[i][j] = 0.0;
...
double SingularValueDecomposition::norm2() const
{
	return s[0];
}

double SingularValueDecomposition::cond() const
{
